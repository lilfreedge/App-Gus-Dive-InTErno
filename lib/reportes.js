...
export function construirResumenHTML({ salidas, tanques, inspecciones, mantenimientos, desde, hasta, incluir }) {
  const totalTanques = (tanques || []).reduce((acc, t) => acc + Number(t.cantidad), 0);
  // Qué secciones se pidieron de verdad (Administración > Reporte semanal
  // por correo) -- antes este resumen siempre mencionaba "salidas" y
  // "tanques llenados" aunque esa sección viniera desmarcada, mostrando
  // "0 salidas registradas" como si ese dato importara. Ahora, si no se
  // pasa `incluir`, se asume todo incluido (compatibilidad hacia atrás).
  const secciones = incluir || { salidas: true, llenados: true, inspecciones: true, mantenimientos: true };
  ...
  // Cada parte del resumen solo aparece si esa sección está marcada en
  // Administración -- ya no se muestra "0 salidas registradas" ni
  // "0 tanques llenados" cuando esa sección directamente no se pidió.
  const resumenPartes = [];
  if (secciones.salidas) {
    resumenPartes.push(`<strong>${(salidas || []).length}</strong> salidas registradas`);
  }
  if (secciones.llenados) {
    resumenPartes.push(`<strong>${totalTanques}</strong> tanques llenados`);
  }
  if (secciones.inspecciones && inspecciones && inspecciones.length > 0) {
    resumenPartes.push(`<strong>${inspecciones.length}</strong> inspecciones visuales`);
  }
  if (secciones.mantenimientos && mantenimientos && mantenimientos.length > 0) {
    resumenPartes.push(`<strong>${mantenimientos.length}</strong> mantenimientos de reguladores`);
  }
      ...
      <div style="padding:16px;">
        <p>${
          resumenPartes.length > 0
            ? resumenPartes.join(" · ")
            : "No hay secciones marcadas para este reporte (revisa Administración &gt; Reporte semanal por correo)."
        }</p>
        ${
          secciones.salidas
            ? (salidas || []).length > 0
...
export async function construirPDF(datos, { desde, hasta, tipo, incluir } = {}) {
  ...
  // Salidas y Llenados son las únicas secciones con una cantidad numérica
  // real (filaComun deja `cantidad = null` en inspecciones/mantenimientos).
  // Si el reporte no incluye ninguna de las dos, la columna "Cantidad"
  // saldría con "—" en todas las filas — se omite y su ancho se reparte
  // entre las demás columnas.
  const incluirNormalizado = typeof activo === "string" ? incluirDeTipo(activo) : activo || {};
  const incluyeCantidad = !!(incluirNormalizado.salidas || incluirNormalizado.llenados);
  ...
  const COLS = [
    ...
    ...(incluyeCantidad ? [{ key: "cantidad", label: "Cantidad", w: 58, align: "right" }] : []),
    { key: "motivoGas", label: col2, w: incluyeCantidad ? 90 : 148, align: "left" },
...
// ============================================================
// Reportes automáticos con varios envíos (V12) -- usado por el cron
// semanal (app/api/cron/reporte-semanal) y por "Reporte instantáneo"
// (app/api/reportes/enviar-ahora), que comparten toda esta lógica.
// ============================================================

// Normaliza app_config.reporte_configs (la lista de envíos, cada uno con
// su propia lista de destinatarios y sus propias secciones) a la forma
// que usa obtenerDatosReporte/construirPDF/construirXLSX. Si todavía no
// hay ningún envío configurado (proyecto nuevo, o no había nada que
// migrar), cae de vuelta a la variable de entorno REPORT_EMAIL_TO con las
// secciones por defecto -- para no dejar de mandar el reporte antes de
// que alguien lo configure desde Reportes.
export function obtenerEnviosConfigurados(config) {
  const configs = Array.isArray(config?.reporte_configs) ? config.reporte_configs : [];

  const envios = configs
    .map((c) => ({
      destinatarios: Array.isArray(c?.destinatarios) ? c.destinatarios.filter(Boolean) : [],
      incluir: {
        salidas: c?.detalles?.salidas !== false,
        llenados: c?.detalles?.llenados !== false,
        inspecciones: !!c?.detalles?.inspecciones,
        mantenimientos: !!c?.detalles?.mantenimientos,
      },
    }))
    .filter((e) => e.destinatarios.length > 0);

  if (envios.length > 0) return envios;
  ...
  return [
    {
      ...
      incluir: { salidas: true, llenados: true, inspecciones: false, mantenimientos: false },
    },
  ];
}

// Manda un correo por cada envío (destinatarios propios + secciones
// propias). Un envío que falla (Resend, o un error consultando sus datos)
// no tumba a los demás -- se reporta aparte en `errores`.
export async function enviarReportesConfigurados(supabase, envios, { asunto, desde, hasta }) {
  const resend = new Resend(process.env.RESEND_API_KEY);
  const resultados = [];
  const errores = [];

  for (const envio of envios) {
    const { incluir, destinatarios } = envio;
    ...
    let datos;
    try {
      datos = await obtenerDatosReporte(supabase, { incluir, desde, hasta });
    } catch (err) {
      errores.push(`${destinatarios.join(", ")}: ${err?.message || String(err)}`);
      continue;
    }

    const [pdfBuffer, xlsxBuffer] = await Promise.all([
      construirPDF(datos, { desde, hasta, incluir }),
      Promise.resolve(construirXLSX(datos, { desde, hasta, incluir })),
    ]);
    const html = construirResumenHTML({ ...datos, desde, hasta, incluir });

    const { error } = await resend.emails.send({
      from: process.env.REPORT_EMAIL_FROM || "Gus Dive <onboarding@resend.dev>",
      to: destinatarios,
      subject: `${asunto} (${desde} a ${hasta})`,
      html,
      attachments: [
        { filename: `gus-dive-reporte_${desde}_a_${hasta}.pdf`, content: pdfBuffer.toString("base64") },
        { filename: `gus-dive-reporte_${desde}_a_${hasta}.xlsx`, content: xlsxBuffer.toString("base64") },
      ],
    });

    if (error) {
      errores.push(`${destinatarios.join(", ")}: ${error?.message || error?.name || JSON.stringify(error)}`);
      continue;
    }

    resultados.push({ destinatarios: destinatarios.length });
  }

  return { resultados, errores };
}

// Datos ya listos para el modal de "Ver reporte" (app/reportes/form-client.js)
// -- mismo cálculo de filas/columnas que el PDF y el Excel, para que la
// vista previa sea fiel a lo que se descarga o se manda por correo.
export function construirVistaPrevia(datos, { desde, hasta, tipo, incluir } = {}) {
  const activo = incluir || tipo;
  const filas = filasReporte(datos);
  const { col1, col2 } = columnasDinamicas(activo);
  const incluirNormalizado = typeof activo === "string" ? incluirDeTipo(activo) : activo || {};
