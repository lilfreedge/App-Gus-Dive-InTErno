...
function filaComun(item, tabla) {
  return {
    ...
    articuloTipo: tabla === "salidas" ? item.articulo : "Llenado de tanque",
...
export function construirXLSX({ salidas, tanques }, { desde, hasta } = {}) {
  const wb = XLSX.utils.book_new();
  const filas = filasReporte({ salidas, tanques });

  const encabezado = ["Folio", "Fecha", "Artículo/Tipo", "Usuario", "Motivo/Gas", "Cantidad"];
  const aoa = [
    ["GUS DIVE CENTER — Reporte semanal"],
    [desde && hasta ? `Del ${desde} al ${hasta}` : ""],
    [],
    encabezado,
    ...filas.map((f) => [f.folio, f.fecha, f.articuloTipo, f.usuario, f.motivoGas, f.cantidad]),
  ];
  ...
  hoja["!cols"] = [{ wch: 8 }, { wch: 16 }, { wch: 26 }, { wch: 20 }, { wch: 16 }, { wch: 10 }];
          ...
          ? `<table cellpadding="6" style="border-collapse:collapse;width:100%;font-size:13px;">
              <thead>
                <tr style="background:#0a3d62;color:#fff;text-align:left;">
                  <th>Folio</th><th>Fecha</th><th>Usuario</th><th>Artículo</th><th>Cant.</th><th>Motivo</th>
                </tr>
              </thead>
...
// ============================================================
export async function construirPDF({ salidas, tanques }, { desde, hasta }) {
  const filas = filasReporte({ salidas, tanques });
  ...
  const COLS = [
    { key: "folio", label: "Folio", w: 48, align: "left" },
    { key: "fecha", label: "Fecha", w: 96, align: "left" },
    { key: "articuloTipo", label: "Artículo/Tipo", w: 150, align: "left" },
    { key: "usuario", label: "Usuario", w: 110, align: "left" },
    { key: "motivoGas", label: "Motivo/Gas", w: 90, align: "left" },
    { key: "cantidad", label: "Cant.", w: 0, align: "right" }, // ancho restante
  ];
  ...
  function nuevaPagina() {
    ...
    if (logoImg) {
      ...
      page.drawImage(logoImg, {
        x: MARGIN,
        y: PAGE_H - BAND_H / 2 - logoH / 2,
        width: logoW,
        height: logoH,
      });
    }

    page.drawText("Reporte semanal", {
      x: MARGIN + 60,
      y: PAGE_H - 30,
      size: 16,
      font: fontBold,
