...
import { formatFecha, formatFechaSolo } from "./format";
...
function filaComun(item, tabla) {
  return {
    folio: item.folio,
    fecha: formatFechaSolo(item.created_at),
    fechaRegistro: formatFecha(item.created_at),
...
export function construirXLSX({ salidas, tanques }, { desde, hasta, tipo } = {}) {
  ...
  const encabezado = ["No.", "Fecha", col1, col2, "Usuario", "Cantidad", "Fecha de registro"];
  const aoa = [
...
export async function construirPDF({ salidas, tanques }, { desde, hasta, tipo }) {
  ...
  const COLS = [
    { key: "folio", label: "No.", w: 34, align: "left" },
    { key: "fecha", label: "Fecha", w: 68, align: "left" },
    { key: "codigoTipo", label: col1, w: 122, align: "left" },
    { key: "motivoGas", label: col2, w: 78, align: "left" },
    { key: "usuario", label: "Usuario", w: 100, align: "left" },
    { key: "cantidad", label: "Cantidad", w: 58, align: "right" },
    { key: "fechaRegistro", label: "Fecha de registro", w: 0, align: "left" }, // ancho restante
  ];
