...
// Igual que obtenerPendientesInterno, pero para la tarjeta "App Clientes"
// de /espacio (23-sep-2026, primera versión real de App Equipos
// Clientes): cuántas órdenes están pendientes por trabajar/despachar.
// Solo consulta si el usuario tiene el permiso `equipos_clientes` (o es
// Titular) -- si no, no tiene sentido mostrarle el número.
export async function obtenerPendientesClientes(supabase, profile) {
  const puedeVer = tieneAcceso(profile, "equipos_clientes");
  if (!puedeVer) return { ordenesPorTrabajar: 0, ordenesPorDespachar: 0 };

  const [porTrabajarRes, porDespacharRes] = await Promise.all([
    supabase
